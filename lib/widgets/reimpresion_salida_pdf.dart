...
class ReimpresionSalidaPdf {
  static Future<void> generateAndPrintPdfSalida({
    ...
    // Separar productos en dos categorías
    final productosNoServicio = productos.where((producto) {
      final entrada =
          (producto['prodUMedEntrada'] ?? '').toString().toLowerCase();
      final salida =
          (producto['prodUMedSalida'] ?? '').toString().toLowerCase();
      return entrada != 'servicio' && salida != 'servicio';
    }).toList();

    final productosServicio = productos.where((producto) {
      final entrada =
          (producto['prodUMedEntrada'] ?? '').toString().toLowerCase();
      final salida =
          (producto['prodUMedSalida'] ?? '').toString().toLowerCase();
    ...
    // Calcular totales
    final totalNoServicio = productosNoServicio.fold<double>(
      0.0,
      (sum, producto) => sum + (producto['precio'] ?? 0.0),
    );

    final totalServicio = productosServicio.fold<double>(
      0.0,
      (sum, producto) => sum + (producto['precio'] ?? 0.0),
    );

    // Convertir totales a letras
    String convertirTotalALetras(double total) {
      final partes = total.toStringAsFixed(2).split('.');
      final entero = int.parse(partes[0]);
      final centavos = partes[1];
      return '${_convertirNumeroALetras(entero)} PESOS $centavos/100 M.N.';
    }

    final totalNoServicioEnLetras = convertirTotalALetras(totalNoServicio);
    final totalServicioEnLetras = convertirTotalALetras(totalServicio);
    ...
    if (productosNoServicio.isNotEmpty) totalPaginas++;
    if (productosServicio.isNotEmpty) totalPaginas++;

    // Función para construir la página
    void addPageToPdf(
        List<Map<String, dynamic>> productosPagina,
        double totalPagina,
        String totalEnLetrasPagina,
        String tituloTabla,
        bool esUltimaPagina) {
      paginaActual++;
          ...
          build: (pw.Context context) {
            return pw.Stack(
              children: [
                        ...
                        pw.TableRow(
                          children: [
                            ...
                            pw.Expanded(
                              ...
                              child: pw.Container(
                                padding: const pw.EdgeInsets.all(3),
                                child: pw.Text('SON: $totalEnLetrasPagina',
                ...
                // Sección de firma al pie de página (solo en la última página)
                if (esUltimaPagina)
                  pw.Positioned(
                    bottom: 30,
                    left: 0,
    ...
    // Agregar páginas según los productos
    if (productosNoServicio.isNotEmpty) {
      addPageToPdf(
          productosNoServicio,
          totalNoServicio,
          totalNoServicioEnLetras,
          'PRODUCTOS',
          productosServicio.isEmpty // Es la última página si no hay servicios
          );
    }

    if (productosServicio.isNotEmpty) {
      addPageToPdf(productosServicio, totalServicio, totalServicioEnLetras,
          'SERVICIOS', true // Siempre es la última página si hay servicios
          );
    }
