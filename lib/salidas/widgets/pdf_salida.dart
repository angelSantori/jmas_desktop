...
Future<bool> validarCamposAntesDeImprimirSalida({
  ...
  if (selectedUserAutoriza == null) {
    showAdvertence(context, 'Debe asignar un usuario que autoriza');
    return false;
  }
...
Future<bool> _guardarPDFSalidaBD({
...
Future<void> generarPdfSalida({
  ...
  try {
    ...
    final pdfBytes = await _generateAndPrintPdfSalidaBytes(
    ...
    final dbSuccess = await _guardarPDFSalidaBD(
...
Future<Uint8List> _generateAndPrintPdfSalidaBytes({
  required String movimiento,
  required String fecha,
  required String folio,
  ...
  // Generar código QR
  final qrBytes = await _generateQrCode(folio);
